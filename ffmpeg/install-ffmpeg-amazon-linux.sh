...
yum -y install glibc-static libwebp-devel.x86_64
...
cd yasm-1.2.0
./configure --prefix="$HOME/ffmpeg_build" --bindir="$HOME/bin" --disable-asm && make install
...
./configure --prefix=$HOME/ffmpeg_build/_build --extra-cflags="-I$HOME/ffmpeg_build/include -static" --extra-ldflags="-L$HOME/ffmpeg_build/lib -static" --bindir=/root/bin --extra-libs=-ldl --enable-version3 --enable-libtheora --enable-libvorbis --enable-libx264 --disable-ffplay --disable-ffserver --enable-gpl --enable-postproc --enable-nonfree --enable-avfilter --enable-pthreads --arch=x86_64 --enable-static --disable-shared pkg_config='pkg-config --static' --enable-encoder=png
